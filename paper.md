...
GixPy assumes the top-left pixel of the detector is the origin of the data array and defines the PONI being defined as the distance from the bottom-left corner of the detector (consistent with pyFAI), as seen in \autoref{fig:poni}.

![An example detector with $10 \times 10$ pixels. The PONI is described by the distance (in meters) from the bottom left corner. A user can convert a PONI in the (row~poni~, column~poni~) format using the `gixpy.convert_to_poni((row, column), (pixel_size_y, pixel_size_x), (rows, columns))` function.\label{fig:poni}](images/pixel-poni.png)
