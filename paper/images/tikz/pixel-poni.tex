...
\begin{tikzpicture}
    ...
    \node[anchor=east] at (0, 0.5*\y) {poni1};

    \draw[stealth-] (-0.5, \y) --++ (0, 1.5);
    \draw[stealth-] (-0.5, \length-.25) --++ (0, -1.5);
    \node[anchor=east] at (0, 2.9) {row$_\text{poni}$};

    \foreach \ii in {0, \x} {
        \draw (\ii, -.1) --++ (0, -.5);
    }
    \node at (0.5*\x, -.4) {poni2};
    ...
    \node at (1.3, 5.4) {column$_\text{poni}$};
    \draw[stealth-] (0.25, 5.4) --++ (.16, 0);
    \draw[stealth-] (\x, 5.4) --++ (-.16, 0);
\end{tikzpicture}
